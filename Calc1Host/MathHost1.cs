...
namespace MathHost
{
    class Program
    {
        static void Main()
        {
            // Assume that current directory is the application folder,
        ...
        private static AddInToken ChooseCalculator(Collection<AddInToken> tokens)
        {
            ...
            Console.WriteLine("Invalid selection: {0}. Please chosse again.", line);
        ...
        private static void RunCalculator(ICalculator calc)
        {
            ...
            while (!line.Equals("exit"))
            {
                ...
                {
                    Console.WriteLine("Invalid command: {0}. Command must be formated: [number] [operation] [number]", line);
                }
